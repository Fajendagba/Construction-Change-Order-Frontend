...
const emit = defineEmits<{
  updated: [changeOrder: ChangeOrder]
  cancel: []
}>()

const { updateChangeOrder } = useChangeOrders()
...
const form = ref<CreateChangeOrderPayload>({
  ...
  cost_code: props.changeOrder.cost_code,
  labor_cost: props.changeOrder.labor_cost,
  material_cost: props.changeOrder.material_cost,
})
...
async function handleSubmit(): Promise<void> {
  ...
  try {
    const updated = await updateChangeOrder(props.projectId, props.changeOrder.id, form.value)
    ...
    if (isAxiosError(err)) {
      if (err.response?.status === 422) {
        ...
        generalError.value = "You don't have permission to edit this change order."
...
<template>
  <div class="bg-white rounded-lg shadow-sm p-6">
    <h2 class="text-lg font-semibold text-gray-900 mb-6">Edit Change Order</h2>
